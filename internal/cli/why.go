...
func runWhy(cmd *cobra.Command, args []string) error {
	...
	if whyNamespace != "" {
		resourceRef.Namespace = whyNamespace
	}
		...
		Solutions: []types.Solution{
			{
				...
				Commands: []string{
					fmt.Sprintf("kubectl describe %s %s -n %s", resourceRef.Kind, resourceRef.Name, whyNamespace),
					fmt.Sprintf("kubectl logs %s -n %s", resourceRef.Name, whyNamespace),
				},
