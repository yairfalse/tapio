...
func (o *Observer) processEvents() {
	...
	for {
		...
		case <-ticker.C:
			...
			// Generate a mock HTTP request event with varying data
			...
			httpEvent := &domain.CollectorEvent{
			...
			// Generate a mock DNS query event with varying data
			...
			dnsEvent := &domain.CollectorEvent{
				...
				EventData: domain.EventDataContainer{
					DNS: &domain.DNSData{
						...
						Answers: []string{
							fmt.Sprintf("10.0.%d.%d", (eventCount*13)%256, (eventCount*17)%256),
						},
					...
					Network: &domain.NetworkData{
						...
						L7Data: &domain.NetworkL7Data{
							...
							DNSData: &domain.DNSQueryData{
								Query:     dnsName,
								QueryType: queryType,
								Answers:   []string{fmt.Sprintf("10.0.%d.%d", (eventCount*13)%256, (eventCount*17)%256)},
							},
						},
					},
					Custom: map[string]string{
