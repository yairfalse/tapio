...
// Expected formats:
// - /sys/fs/cgroup/kubepods/burstable/pod<POD_UID>/<CONTAINER_ID>/
// - /sys/fs/cgroup/kubepods/besteffort/pod<POD_UID>/<CONTAINER_ID>/
// - /sys/fs/cgroup/kubepods/pod<POD_UID>/<CONTAINER_ID>/
// - /docker/<CONTAINER_ID>/
// - /containerd/<CONTAINER_ID>/
static __always_inline void extract_container_id_from_cgroup(u64 cgroup_id, char *container_id, int max_len) {
    // Initialize container_id as empty
    if (max_len > 0) {
        container_id[0] = '\0';
    }
    
    // We can't directly read cgroup path in eBPF, but we can use cgroup_id
    // as a key to look up cached metadata or derive container info
    // For now, we'll use cgroup_id as a unique identifier
    
    // In production, this would involve:
    // 1. Reading from /proc/<pid>/cgroup or using BPF helpers
    // 2. Parsing the path to extract container ID
    // 3. Handling different container runtime formats
    
    // Temporary: Use cgroup_id as container identifier
    // This will be replaced with actual path parsing when cgroup helpers are available
    if (cgroup_id != 0 && max_len > 16) {
        // Format cgroup_id as hex string for container ID placeholder
        char hex_chars[] = "0123456789abcdef";
        int i;
        for (i = 0; i < 16 && i < max_len - 1; i++) {
            u8 nibble = (cgroup_id >> (60 - i * 4)) & 0xF;
            container_id[i] = hex_chars[nibble];
        }
        container_id[i] = '\0';
    }
}
...
int trace_oom_kill(struct pt_regs *ctx) {
    ...
    if (meta) {
        ...
        // Extract container ID from cgroup if metadata not available
        extract_container_id_from_cgroup(cgroup_id, event->container_id, MAX_CONTAINER_ID_LEN);
    }
