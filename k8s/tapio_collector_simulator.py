...
class OTLPExporter:
    ...
    def __init__(self):
        ...
        # Create resource with enhanced Kubernetes context
        resource = Resource.create({
            ResourceAttributes.SERVICE_NAME: SERVICE_NAME,
            ResourceAttributes.SERVICE_VERSION: SERVICE_VERSION,
            ResourceAttributes.SERVICE_NAMESPACE: "ebpf.monitoring",
            ...
            "k8s.pod.ip": POD_IP,
            "k8s.container.name": "tapio-collector",
            "host.ip": HOST_IP,
            "host.name": NODE_NAME,
            "component.type": "ebpf-collector",
            "tapio.collector.type": "simulator",
        })
        ...
        try:
            ...
            # Create metrics with cleaner naming
            self.event_counter = self.meter.create_counter(
                name="tapio.collector.events_total",
                description="Total number of eBPF events generated by Tapio collector",
                unit="1"
            )
            
            self.process_gauge = self.meter.create_up_down_counter(
                name="tapio.collector.processes_active",
                description="Number of active processes being monitored by Tapio",
                unit="1"
            )
            
            self.network_connections = self.meter.create_up_down_counter(
                name="tapio.collector.connections_active",
                description="Number of active network connections monitored by Tapio",
                unit="1"
            )
            
            self.event_latency = self.meter.create_histogram(
                name="tapio.collector.event_latency_ms",
                description="Latency distribution of event processing in milliseconds",
                unit="ms"
            )
    ...
    def export_process_event(self, event: ProcessEvent):
        ...
        # Create trace span with cleaner naming
        ...
        with self.tracer.start_as_current_span(span_name) as span:
            span.set_attributes({
                ...
                "service.name": f"{event.pod_namespace}.{event.comm}",
                "service.instance.id": f"{event.pod_name}-{event.pid}",
                "event.category": "process",
                "event.action": event.event_type,
                "tapio.collector.source": "ebpf.process",
            })
    ...
    def export_network_event(self, event: NetworkEvent):
        ...
        # Create trace span with cleaner naming
        span_name = f"network/{event.event_type}"
        with self.tracer.start_as_current_span(span_name) as span:
            span.set_attributes({
                ...
                "k8s.pod.name": event.pod_name,
                "k8s.namespace.name": event.pod_namespace,
                "service.name": f"{event.pod_namespace}.{event.service_name}",
                "service.instance.id": f"{event.pod_name}-{event.pid}",
                "event.category": "network",
                "event.action": event.event_type,
                "tapio.collector.source": "ebpf.network",
                "network.connection.type": "client" if event.event_type == "connect" else "server" if event.event_type == "accept" else "data",
            })
    ...
    def export_dns_event(self, event: DNSEvent):
        ...
        # Create trace span with cleaner naming
        span_name = f"dns/{event.event_type}"
        with self.tracer.start_as_current_span(span_name) as span:
            span.set_attributes({
                ...
                "k8s.pod.name": event.pod_name,
                "k8s.namespace.name": event.pod_namespace,
                "service.name": f"{event.pod_namespace}.dns",
                "service.instance.id": f"{event.pod_name}-{event.pid}",
                "event.category": "network",
                "event.action": f"dns_{event.event_type}",
                "tapio.collector.source": "ebpf.dns",
                "dns.query.class": "IN",
            })
    ...
    def export_security_event(self, event: SecurityEvent):
        ...
        # Create trace span with cleaner naming
        span_name = f"security/{event.event_type}"
        with self.tracer.start_as_current_span(span_name) as span:
            span.set_attributes({
                ...
                "k8s.pod.name": event.pod_name,
                "k8s.namespace.name": event.pod_namespace,
                "service.name": f"{event.pod_namespace}.security",
                "service.instance.id": f"{event.pod_name}-{event.pid}",
                "event.category": "security",
                "event.action": event.event_type,
                "event.outcome": "success" if event.result == 0 else "failure",
                "tapio.collector.source": "ebpf.security",
                "security.attack.technique": "T1055" if event.syscall_name == "ptrace" else None,
            })
    ...
    def export_http_event(self, event: HTTPEvent):
        ...
        # Create trace span with cleaner naming
        span_name = f"http/{event.method} {event.url}"
        with self.tracer.start_as_current_span(span_name) as span:
            span.set_attributes({
                ...
                "tapio.collector.source": "ebpf.http",
                "http.flavor": "1.1",
                "http.scheme": "http" if event.status_code != 443 else "https",
            })
...
class TapioCollectorSimulator:
    ...
    async def _generate_single_event(self, event_type: str):
        """Generate a single event of the specified type with trace correlation"""
        try:
            # Create a root span for the event generation session to establish hierarchy
            if OTEL_AVAILABLE and self.otlp_exporter.tracer:
                with self.otlp_exporter.tracer.start_as_current_span(
                    f"tapio/collector/session",
                    attributes={
                        "tapio.session.type": "event_generation",
                        "tapio.event.category": event_type,
                        "k8s.node.name": NODE_NAME,
                        "k8s.cluster.name": CLUSTER_NAME,
                    }
                ) as root_span:
                    await self._generate_event_with_context(event_type, root_span)
            else:
                await self._generate_event_with_context(event_type, None)
    ...
    async def _generate_event_with_context(self, event_type: str, parent_span):
        """Generate event with proper trace context"""
