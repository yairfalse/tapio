...
func CPUThrottleDetection() *events_correlation.Rule {
	return &events_correlation.Rule{
		...
		Evaluate: func(ctx *events_correlation.Context) *events_correlation.Result {
			...
			throttleEvents := ctx.GetEvents(events_correlation.Filter{
			...
			perfEvents := ctx.GetEvents(events_correlation.Filter{
			...
			timeoutEvents := ctx.GetEvents(events_correlation.Filter{
				Source: events_correlation.SourceEBPF,
			...
			correlatedPerfEvents := make([]events_correlation.Event, 0)
...
func CPUContentionDetection() *events_correlation.Rule {
	return &events_correlation.Rule{
		...
		Evaluate: func(ctx *events_correlation.Context) *events_correlation.Result {
			...
			nodeThrottleMap := make(map[string][]events_correlation.Event)
			for _, event := range throttleEvents {
			...
			evidence := events_correlation.Evidence{
				...
				Entities: []events_correlation.Entity{
					{
						Type: "node",
						Name: contentionNode,
					},
...
func HighCPUUtilizationPattern() *events_correlation.Rule {
	return &events_correlation.Rule{
