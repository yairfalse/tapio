...
func isControllerManagerPod(pod PodInfo) bool {
	// Check by label
...
func isSchedulerPod(pod PodInfo) bool {
	// Check by label
...
func isAdmissionWebhook(pod PodInfo) bool {
	// Check for common webhook labels
	if _, ok := pod.Labels["webhook"]; ok {
		return true
	}
	// Check for admission webhook annotations
	// TODO: Add support for pod annotations when PodInfo is extended
...
func getPodCertificateExpiry(pod PodInfo, k8sData *correlation.KubernetesData) (string, bool) {
...
func isCloudControllerPodHelper(pod PodInfo) bool {
	// Check by label
