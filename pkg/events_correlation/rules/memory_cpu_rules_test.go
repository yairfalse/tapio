...
func TestMemoryPressureCascadeRule(t *testing.T) {
	tests := []struct {
		...
		expectedSev    events_correlation.Severity
	}{
		{
			name: "no OOM events",
			events: []events_correlation.Event{
		...
		},
		{
			...
			events: func() []events_correlation.Event {
				baseTime := time.Now()
			...
			expectedResult: true,
			expectedConf:   0.90, // Actual confidence from rule
			expectedSev:    events_correlation.SeverityMedium,
		},
		{
			...
			events: func() []events_correlation.Event {
				...
				for i := 1; i <= 6; i++ {
					events = append(events, events_correlation.Event{
			...
			expectedResult: true,
			expectedSev:    events_correlation.SeverityCritical,
		},
	}
...
func TestMemoryLeakDetectionRule(t *testing.T) {
	tests := []struct {
		name           string
		metricSeries   events_correlation.MetricSeries
		...
		},
		{
			...
			metricSeries: events_correlation.MetricSeries{
				Name: "container.memory.usage",
				Points: func() []events_correlation.MetricPoint {
					points := make([]events_correlation.MetricPoint, 15)
					baseTime := time.Now()
					for i := range points {
						points[i] = events_correlation.MetricPoint{
		...
		},
		{
			...
			metricSeries: events_correlation.MetricSeries{
				Name: "container.memory.usage",
				Points: func() []events_correlation.MetricPoint {
					points := make([]events_correlation.MetricPoint, 20)
					baseTime := time.Now()
					for i := range points {
						points[i] = events_correlation.MetricPoint{
							Timestamp: baseTime.Add(time.Duration(i) * time.Minute),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx := createTestContextWithMetricSeries([]events_correlation.Event{}, map[string]events_correlation.MetricSeries{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectedResult {
				...
				if tt.expectCritical && result.Severity != events_correlation.SeverityCritical {
...
func TestRuleRegistryFunctions(t *testing.T) {
	...
	t.Run("GetRulesByCategory", func(t *testing.T) {
		resourceRules := GetRulesByCategory(events_correlation.CategoryResource)
		if len(resourceRules) == 0 {
			t.Error("expected to find resource category rules")
		}

		performanceRules := GetRulesByCategory(events_correlation.CategoryPerformance)
...
func createTestContext(events []events_correlation.Event) *events_correlation.Context {
	return createTestContextWithMetrics(events, nil)
}

func createTestContextWithMetrics(events []events_correlation.Event, metrics map[string]float64) *events_correlation.Context {
	// Use the existing NewTestContext function
	ctx := events_correlation.NewTestContext(events)

	// Add metric values if provided
	if metrics != nil {
		for name, value := range metrics {
			// Create a simple metric series with one point
			series := events_correlation.MetricSeries{
				Name: name,
				Points: []events_correlation.MetricPoint{
					{
						Timestamp: time.Now(),
...
func createTestContextWithMetricSeries(events []events_correlation.Event, series map[string]events_correlation.MetricSeries, values map[string]float64) *events_correlation.Context {
