...
func MemoryPressureCascade() *events_correlation.Rule {
	return &events_correlation.Rule{
		...
		Evaluate: func(ctx *events_correlation.Context) *events_correlation.Result {
			...
			oomEvents := ctx.GetEvents(events_correlation.Filter{
			...
			podRestarts := ctx.GetEvents(events_correlation.Filter{
			...
			correlatedRestarts := make([]events_correlation.Event, 0)
			...
			serviceFailures := ctx.GetEvents(events_correlation.Filter{
			...
			evidence := events_correlation.Evidence{
				...
				Timeline: timeline,
				Entities: []events_correlation.Entity{firstOOM.Entity},
...
func MemoryLeakDetection() *events_correlation.Rule {
	return &events_correlation.Rule{
		...
		Evaluate: func(ctx *events_correlation.Context) *events_correlation.Result {
			...
			recentRestarts := ctx.GetEvents(events_correlation.Filter{
				Source: events_correlation.SourceKubernetes,
				Type:   "pod.restart",
...
func ContainerOOMPrediction() *events_correlation.Rule {
	return &events_correlation.Rule{
		...
		Evaluate: func(ctx *events_correlation.Context) *events_correlation.Result {
			...
			recentSeries := events_correlation.MetricSeries{
				Name:   "recent_memory",
				Points: recentPoints,
			}
