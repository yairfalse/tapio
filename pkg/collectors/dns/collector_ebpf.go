...
func (c *Collector) startEBPF() error {
	...
	if os.Getenv("TAPIO_MOCK_MODE") == "true" {
		c.mockMode = true
		c.logger.Info("DNS collector running in MOCK MODE")
...
func (c *Collector) processBPFEvent(ctx context.Context, data []byte) error {
	...
	collectorEvent, err := c.convertBPFEventToCollectorEvent(&bpfEvent)
	if err != nil {
...
// convertBPFEventToCollectorEvent converts a BPF DNS event to a CollectorEvent
func (c *Collector) convertBPFEventToCollectorEvent(bpfEvent *BPFDNSEvent) (*domain.CollectorEvent, error) {
	// Extract query name
