...
func main() {
	...
	collector, err := dns.NewCollector("dns-test", config)
	if err != nil {
		logger.Fatal("Failed to create DNS collector", zap.Error(err))
	}
	...
	if err := collector.Start(ctx); err != nil {
		logger.Fatal("Failed to start DNS collector", zap.Error(err))
	}

	logger.Info("DNS collector started successfully")
	...
	// Print collector stats periodically
	...
	logger.Info("DNS collector is running. Press Ctrl+C to stop.")
	...
	logger.Info("DNS collector stopped")
}
