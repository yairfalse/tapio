...
func (p *ContainerEventParser) Parse(entry *JournalEntry) *unified.Event {
...
func (p *ContainerEventParser) createContainerEvent(entry *JournalEntry, failureType string, details map[string]interface{}) *unified.Event {
...
func (p *ContainerEventParser) determineSeverity(failureType string, details map[string]interface{}) unified.Severity {
	// Critical failures
	...
	if condition, ok := details["error_condition"].(string); ok {
		...
		case "rate_limit", "timeout_error":
			return unified.SeverityWarning
		}
	}
...
func (p *ContainerEventParser) calculateImportance(failureType string, severity unified.Severity) float64 {
	...
	if severity == unified.SeverityCritical {
		base = max(base, 0.9)
	}
...
func (p *ContainerEventParser) buildContext(entry *JournalEntry, details map[string]interface{}) *unified.EventContext {
	ctx := &unified.EventContext{
		Node:        entry.Hostname,
...
func (p *ContainerEventParser) createActionableItem(failureType string, details map[string]interface{}) *unified.ActionableItem {
	action := &unified.ActionableItem{
		Title:       "Container Remediation",
		Description: "Container failure detected",
