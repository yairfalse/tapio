...
// CRIIntegration handles integration between CRI and CRI-eBPF collectors
...
func NewCRIIntegration(ebpfCollector *Collector) *CRIIntegration {
	logger, _ := zap.NewProduction()
	return &CRIIntegration{
		ebpfCollector: ebpfCollector,
		logger:        logger.Named("cri-ebpf-integration"),
	}
}

// HandleCRIEvent processes events from the CRI collector
...
func (ci *CRIIntegration) handleContainerStart(ctx context.Context, event *domain.CollectorEvent) error {
	...
	if err := ci.ebpfCollector.OnContainerStart(containerID, k8sMeta); err != nil {
...
func (ci *CRIIntegration) handleContainerStop(ctx context.Context, event *domain.CollectorEvent) error {
	...
	if err := ci.ebpfCollector.OnContainerStop(containerID); err != nil {
