...
func (sm *ServiceMonitor) determineSeverity(oldState, newState string, info *ServiceInfo) collectors.Severity {
	...
	if info.IsCritical || info.IsContainerRuntime {
		...
		if newState == "inactive" && oldState == "active" {
			return collectors.SeverityHigh
		}
	}
	...
	case "inactive":
		if oldState == "active" {
			return collectors.SeverityMedium
		}
		return collectors.SeverityLow
	case "active":
		return collectors.SeverityLow
	default:
		return collectors.SeverityDebug
	}
}
...
func (sm *ServiceMonitor) shouldFilterEvent(event *ServiceEvent) bool {
	// Filter by severity
	if event.Severity < collectors.SeverityLow {
		return true
	}
