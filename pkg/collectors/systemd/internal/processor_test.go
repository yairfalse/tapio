	...
	tests := []struct {
		...
		},
		{
			...
			validate: func(t *testing.T, event *domain.UnifiedEvent, err error) {
				...
				// PostgreSQL is not customer-facing so affected users should be 0
				if event.Impact.AffectedUsers != 0 {
					t.Errorf("postgresql is not customer-facing, expected 0 affected users, got %d", event.Impact.AffectedUsers)
				}
			},
		},
		{
...
func TestSystemdEventProcessor_BusinessImpact(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			minExpected: 0.89, // Base (0.1) + Critical (0.5) + Failure (0.3) = 0.9, slightly below for floating point
			maxExpected: 1.0,
		},
		{
