...
type YAMLConfig struct {
	...
	Collectors   map[string]CollectorYAMLConfig `yaml:"collectors"`
}
...
// CollectorYAMLConfig holds per-collector configuration
type CollectorYAMLConfig struct {
	Enabled bool                `yaml:"enabled"`
	Config  CollectorConfigData `yaml:"config"`
}

// CollectorConfigData holds typed collector configuration
type CollectorConfigData struct {
	// Common fields
	BufferSize   int    `yaml:"buffer_size"`
...
func ValidateYAMLConfig(config *YAMLConfig) error {
	...
	hasEnabled := false
	for name, cfg := range config.Collectors {
		if cfg.Enabled {
			hasEnabled = true
			break
		}
	...
	if !hasEnabled {
		return fmt.Errorf("no collectors are enabled")
	}
...
// GetCollectorConfig extracts configuration for a specific collector
func (c *YAMLConfig) GetCollectorConfig(name string) (*CollectorConfigData, bool) {
	if cfg, exists := c.Collectors[name]; exists && cfg.Enabled {
...
func (c *YAMLConfig) IsCollectorEnabled(name string) bool {
	if cfg, exists := c.Collectors[name]; exists {
		return cfg.Enabled
	}
	return false
}
