...
// BackpressureController manages load shedding and backpressure for CNI events
type BackpressureController struct {
	...
	// Random generator for shedding decisions
	rng *rand.Rand
}
...
// EventPriority represents CNI event priority
...
const (
	...
	PriorityNormal
	PriorityHigh
	PriorityCritical // Never shed (e.g., pod creation/deletion)
)
...
func (bc *BackpressureController) ShouldAccept(eventType core.CNIEventType) bool {
	priority := bc.getEventPriority(eventType)
	return bc.shouldAcceptWithPriority(priority)
}

// ShouldAcceptRaw determines if a raw CNI event should be accepted
func (bc *BackpressureController) ShouldAcceptRaw(event *core.CNIRawEvent) bool {
	priority := bc.getRawEventPriority(event)
	return bc.shouldAcceptWithPriority(priority)
}

// shouldAcceptWithPriority determines if an event should be accepted based on priority
func (bc *BackpressureController) shouldAcceptWithPriority(priority EventPriority) bool {
...
// getEventPriority determines priority based on CNI event type
func (bc *BackpressureController) getEventPriority(eventType core.CNIEventType) EventPriority {
	switch eventType {
	case core.CNIEventTypeIPAllocation, core.CNIEventTypeIPDeallocation:
		return PriorityCritical // Pod lifecycle events are critical
	case core.CNIEventTypeInterfaceSetup, core.CNIEventTypeInterfaceTeardown:
		return PriorityHigh
	case core.CNIEventTypeError:
		return PriorityHigh // Errors are important for debugging
	case core.CNIEventTypeHealthCheck:
		return PriorityLow // Health checks can be shed under load
	default:
		return PriorityNormal
	}
}

// getRawEventPriority determines priority from raw CNI event
func (bc *BackpressureController) getRawEventPriority(event *core.CNIRawEvent) EventPriority {
	// Critical operations that affect pod lifecycle
	if event.Operation == core.CNIOperationAdd || event.Operation == core.CNIOperationDel {
		return PriorityCritical
	}

	// Failed operations are high priority for debugging
	if !event.Success {
		return PriorityHigh
	}

	// Health checks are low priority
	if event.Operation == core.CNIOperationCheck {
		return PriorityLow
	}
...
func (bc *BackpressureController) GetMetricsMap() map[string]interface{} {
	bc.mu.RLock()
	defer bc.mu.RUnlock()

	metrics := bc.GetMetrics()
	return map[string]interface{}{
		"events_accepted":    metrics.EventsAccepted,
		"events_shed":        metrics.EventsShed,
		"load_transitions":   metrics.LoadTransitions,
		"current_shed_rate":  metrics.CurrentShedRate,
		"current_load_level": metrics.CurrentLoadLevel,
		"time_in_high_load":  metrics.TimeInHighLoad.Seconds(),
	}
}
