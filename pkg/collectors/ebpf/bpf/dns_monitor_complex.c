...
static __u16 (*bpf_ntohs)(__u16 netshort) = (void *) 148;
static __u32 (*bpf_ntohl)(__u32 netlong) = (void *) 149;

// BPF Core Read helpers
#define BPF_CORE_READ_INTO(dst, src, a, ...) \
    bpf_probe_read_kernel(dst, sizeof(*(dst)), &((src)->a))

#define BPF_CORE_READ(src, a, ...) \
    ({ typeof((src)->a) __r; \
       BPF_CORE_READ_INTO(&__r, src, a); \
       __r; })

// Kprobe/kretprobe macros
...
int getaddrinfo_entry(struct pt_regs *ctx) {
    const char *node = (const char *)ctx->di; // First argument
    u64 id = bpf_get_current_pid_tgid();
    u32 pid = id >> 32;
...
int getaddrinfo_exit(struct pt_regs *ctx) {
    int ret = (int)ctx->ax; // Return value
    u64 id = bpf_get_current_pid_tgid();
    u32 pid = id >> 32;
    u32 tgid = id;
    u32 uid = bpf_get_current_uid_gid();
...
int gethostbyname_entry(struct pt_regs *ctx) {
    const char *name = (const char *)ctx->di; // First argument
    u64 id = bpf_get_current_pid_tgid();
    u32 pid = id >> 32;
...
int gethostbyname_exit(struct pt_regs *ctx) {
    struct hostent *ret = (struct hostent *)ctx->ax; // Return value
    u64 id = bpf_get_current_pid_tgid();
    u32 pid = id >> 32;
    u32 tgid = id;
    u32 uid = bpf_get_current_uid_gid();
