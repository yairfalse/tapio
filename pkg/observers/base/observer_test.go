...
func TestBaseObserver(t *testing.T) {
	t.Run("initialization", func(t *testing.T) {
		bc := NewBaseObserver("test-observer", 5*time.Minute)

		assert.Equal(t, "test-observer", bc.GetName())
	...
	t.Run("health degraded on stale events", func(t *testing.T) {
		bc := NewBaseObserver("test-observer", 100*time.Millisecond) // Short timeout
		bc.RecordEvent()
...
func BenchmarkBaseObserver(b *testing.B) {
	bc := NewBaseObserver("bench", 5*time.Minute)
