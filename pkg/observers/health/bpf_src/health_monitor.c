 ...
 * Captures critical system health issues: disk space, memory, connections
...
/* Critical health indicators we monitor */
#define ENOSPC 28     /* Disk space exhaustion */
#define ENOMEM 12     /* Memory exhaustion */
...
#define EMFILE 24     /* File descriptor exhaustion */
#define EDQUOT 122    /* Disk quota exhaustion */
...
static __always_inline void get_task_info(struct health_event *event)
{
...
/* Syscall exit tracepoint - capture health issues */
...
int trace_sys_exit(struct trace_event_raw_sys_exit *ctx)
{
    ...
    /* Allocate event */
    struct health_event *event = bpf_ringbuf_reserve(&events, sizeof(*event), 0);
    if (!event)
        goto cleanup;
