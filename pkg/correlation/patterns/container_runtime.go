...
func (crfd *ContainerRuntimeFailureDetector) Category() correlation.Category {
	return correlation.CategoryReliability
}

// Configure updates the detector configuration
func (crfd *ContainerRuntimeFailureDetector) Configure(config PatternConfig) error {
	crfd.config = config
	return nil
}

// GetConfig returns the current configuration
func (crfd *ContainerRuntimeFailureDetector) GetConfig() PatternConfig {
	return crfd.config
}
...
func (crfd *ContainerRuntimeFailureDetector) Detect(ctx context.Context, events []correlation.Event, metrics map[string]correlation.MetricSeries) (*PatternResult, error) {
	start := time.Now()
	defer func() {
...
func (crfd *ContainerRuntimeFailureDetector) filterRuntimeEvents(events []correlation.Event) []correlation.Event {
	var runtimeEvents []correlation.Event
...
func (crfd *ContainerRuntimeFailureDetector) isRuntimeEvent(event correlation.Event) bool {
...
func (crfd *ContainerRuntimeFailureDetector) updateRuntimeStates(events []correlation.Event, metrics map[string]correlation.MetricSeries) {
...
func (crfd *ContainerRuntimeFailureDetector) updateNodeStates(events []correlation.Event, metrics map[string]correlation.MetricSeries) {
	cutoff := time.Now().Add(-crfd.config.LookbackWindow)
...
func (crfd *ContainerRuntimeFailureDetector) detectRuntimeAnomalies(events []correlation.Event, metrics map[string]correlation.MetricSeries) []*RuntimeAnomaly {
...
func (crfd *ContainerRuntimeFailureDetector) detectStartupAnomalies(events []correlation.Event) []*RuntimeAnomaly {
	var anomalies []*RuntimeAnomaly
	
	// Group events by node and analyze startup patterns
	nodeEvents := make(map[string][]correlation.Event)
	for _, event := range events {
...
func (crfd *ContainerRuntimeFailureDetector) detectImagePullAnomalies(events []correlation.Event) []*RuntimeAnomaly {
	var anomalies []*RuntimeAnomaly
...
func (crfd *ContainerRuntimeFailureDetector) detectNetworkingAnomalies(events []correlation.Event) []*RuntimeAnomaly {
	var anomalies []*RuntimeAnomaly
...
func (crfd *ContainerRuntimeFailureDetector) detectStorageAnomalies(events []correlation.Event) []*RuntimeAnomaly {
	var anomalies []*RuntimeAnomaly
...
func (crfd *ContainerRuntimeFailureDetector) detectResourceAnomalies(events []correlation.Event, metrics map[string]correlation.MetricSeries) []*RuntimeAnomaly {
	var anomalies []*RuntimeAnomaly
...
func (crfd *ContainerRuntimeFailureDetector) detectRuntimeDaemonAnomalies(events []correlation.Event, metrics map[string]correlation.MetricSeries) []*RuntimeAnomaly {
	var anomalies []*RuntimeAnomaly
...
func (crfd *ContainerRuntimeFailureDetector) detectKubeletAnomalies(events []correlation.Event, metrics map[string]correlation.MetricSeries) []*RuntimeAnomaly {
	var anomalies []*RuntimeAnomaly
...
func (crfd *ContainerRuntimeFailureDetector) extractRuntimeType(event correlation.Event) string {
	if event.Attributes != nil {
...
func (crfd *ContainerRuntimeFailureDetector) extractRuntimeTypeFromEvents(events []correlation.Event) string {
	for _, event := range events {
...
func (crfd *ContainerRuntimeFailureDetector) updateRuntimeStateFromEvent(runtime *RuntimeState, event correlation.Event) {
...
func (crfd *ContainerRuntimeFailureDetector) updateNodeStateFromEvent(nodeState *NodeRuntimeState, event correlation.Event) {
...
func (crfd *ContainerRuntimeFailureDetector) updateRuntimeStatesFromMetrics(metrics map[string]correlation.MetricSeries) {
	// Implementation for updating runtime states from metrics
}

func (crfd *ContainerRuntimeFailureDetector) updateNodeStatesFromMetrics(metrics map[string]correlation.MetricSeries) {
	// Implementation for updating node states from metrics
}

// Event type checkers
func (crfd *ContainerRuntimeFailureDetector) isStartupEvent(event correlation.Event) bool {
...
func (crfd *ContainerRuntimeFailureDetector) isImagePullEvent(event correlation.Event) bool {
	return event.Type == "image_pull_failed" || event.Type == "image_pull_backoff"
}

func (crfd *ContainerRuntimeFailureDetector) isImagePullFailure(event correlation.Event) bool {
	return event.Type == "image_pull_failed"
}

func (crfd *ContainerRuntimeFailureDetector) isNetworkingEvent(event correlation.Event) bool {
	return event.Type == "network_not_ready" || strings.Contains(event.Type, "network")
}

func (crfd *ContainerRuntimeFailureDetector) isNetworkFailure(event correlation.Event) bool {
	return strings.Contains(event.Type, "network") && strings.Contains(event.Type, "failed")
}

func (crfd *ContainerRuntimeFailureDetector) isStorageEvent(event correlation.Event) bool {
	return event.Type == "volume_mount_failed" || strings.Contains(event.Type, "storage")
}

func (crfd *ContainerRuntimeFailureDetector) isStorageFailure(event correlation.Event) bool {
	return strings.Contains(event.Type, "volume") && strings.Contains(event.Type, "failed")
}

func (crfd *ContainerRuntimeFailureDetector) isResourceEvent(event correlation.Event) bool {
	return event.Type == "resource_quota_exceeded" || strings.Contains(event.Type, "resource")
}

func (crfd *ContainerRuntimeFailureDetector) isResourceFailure(event correlation.Event) bool {
	return strings.Contains(event.Type, "resource") && (strings.Contains(event.Type, "failed") || strings.Contains(event.Type, "exceeded"))
}

func (crfd *ContainerRuntimeFailureDetector) isRuntimeDaemonEvent(event correlation.Event) bool {
	return event.Type == "runtime_not_ready" || event.Type == "runtime_unhealthy" || event.Type == "container_runtime_error"
}

func (crfd *ContainerRuntimeFailureDetector) isRuntimeDaemonFailure(event correlation.Event) bool {
	return strings.Contains(event.Type, "runtime") && (strings.Contains(event.Type, "failed") || strings.Contains(event.Type, "unhealthy"))
}

func (crfd *ContainerRuntimeFailureDetector) isKubeletEvent(event correlation.Event) bool {
	return event.Type == "kubelet_not_ready" || strings.Contains(event.Type, "kubelet")
}

func (crfd *ContainerRuntimeFailureDetector) isKubeletFailure(event correlation.Event) bool {
	return strings.Contains(event.Type, "kubelet") && strings.Contains(event.Type, "not_ready")
}

func (crfd *ContainerRuntimeFailureDetector) isFailureEvent(event correlation.Event) bool {
	return strings.Contains(event.Type, "failed") || strings.Contains(event.Type, "error") || strings.Contains(event.Type, "unhealthy")
}

// Extraction helpers
func (crfd *ContainerRuntimeFailureDetector) extractStartupTime(event correlation.Event) float64 {
	if event.Attributes != nil {
...
func (crfd *ContainerRuntimeFailureDetector) extractImageName(event correlation.Event) string {
	if event.Attributes != nil {
...
func (crfd *ContainerRuntimeFailureDetector) extractPodName(event correlation.Event) string {
	if event.Entity.Type == "pod" {
...
func (crfd *ContainerRuntimeFailureDetector) analyzeFailurePattern(anomalies []*RuntimeAnomaly, events []correlation.Event) *RuntimeFailureAnalysis {
...
func (crfd *ContainerRuntimeFailureDetector) determineSeverity(analysis *RuntimeFailureAnalysis, impact ImpactAssessment) correlation.Severity {
...
func (crfd *ContainerRuntimeFailureDetector) extractAffectedEntities(anomalies []*RuntimeAnomaly) []correlation.Entity {
...
func (crfd *ContainerRuntimeFailureDetector) assessDataQuality(events []correlation.Event, metrics map[string]correlation.MetricSeries) float64 {
	return 0.9
}