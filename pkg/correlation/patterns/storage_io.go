...
func (sibd *StorageIOBottleneckDetector) Category() correlation.Category {
	return correlation.CategoryResource
}

// Configure updates the detector configuration
func (sibd *StorageIOBottleneckDetector) Configure(config PatternConfig) error {
	sibd.config = config
	return nil
}

// GetConfig returns the current configuration
func (sibd *StorageIOBottleneckDetector) GetConfig() PatternConfig {
	return sibd.config
}
...
func (sibd *StorageIOBottleneckDetector) Detect(ctx context.Context, events []correlation.Event, metrics map[string]correlation.MetricSeries) (*PatternResult, error) {
	start := time.Now()
	defer func() {
...
func (sibd *StorageIOBottleneckDetector) filterStorageEvents(events []correlation.Event) []correlation.Event {
	var storageEvents []correlation.Event
...
func (sibd *StorageIOBottleneckDetector) isStorageEvent(event correlation.Event) bool {
...
func (sibd *StorageIOBottleneckDetector) updateStorageDevices(events []correlation.Event, metrics map[string]correlation.MetricSeries) {
...
func (sibd *StorageIOBottleneckDetector) updateVolumeMappings(events []correlation.Event) {
	for _, event := range events {
		if event.Entity.Type != "persistentvolume" && event.Entity.Type != "persistentvolumeclaim" {
			continue
		}
...
func (sibd *StorageIOBottleneckDetector) detectStorageAnomalies(events []correlation.Event, metrics map[string]correlation.MetricSeries) []*StorageAnomaly {
...
func (sibd *StorageIOBottleneckDetector) detectIOPSAnomalies(metrics map[string]correlation.MetricSeries) []*StorageAnomaly {
	var anomalies []*StorageAnomaly
...
func (sibd *StorageIOBottleneckDetector) detectLatencyAnomalies(metrics map[string]correlation.MetricSeries) []*StorageAnomaly {
	var anomalies []*StorageAnomaly
...
func (sibd *StorageIOBottleneckDetector) detectThroughputAnomalies(metrics map[string]correlation.MetricSeries) []*StorageAnomaly {
	var anomalies []*StorageAnomaly
...
func (sibd *StorageIOBottleneckDetector) detectCapacityAnomalies(metrics map[string]correlation.MetricSeries) []*StorageAnomaly {
	var anomalies []*StorageAnomaly
...
func (sibd *StorageIOBottleneckDetector) detectIOErrorAnomalies(events []correlation.Event) []*StorageAnomaly {
	var anomalies []*StorageAnomaly
	
	// Group error events by device and time windows
	deviceErrors := make(map[string][]correlation.Event)
	for _, event := range events {
		if !sibd.isStorageErrorEvent(event) {
			continue
		}
...
func (sibd *StorageIOBottleneckDetector) detectQueueDepthAnomalies(metrics map[string]correlation.MetricSeries) []*StorageAnomaly {
	var anomalies []*StorageAnomaly
...
func (sibd *StorageIOBottleneckDetector) analyzeBottleneckPattern(anomalies []*StorageAnomaly, events []correlation.Event) *BottleneckAnalysis {
...
func (sibd *StorageIOBottleneckDetector) extractDeviceIDFromEvent(event correlation.Event) string {
	if event.Attributes != nil {
...
func (sibd *StorageIOBottleneckDetector) isStorageErrorEvent(event correlation.Event) bool {
...
func (sibd *StorageIOBottleneckDetector) isReadError(event correlation.Event) bool {
	return event.Type == "disk_read_error"
}

func (sibd *StorageIOBottleneckDetector) isWriteError(event correlation.Event) bool {
...
func (sibd *StorageIOBottleneckDetector) determineSeverity(analysis *BottleneckAnalysis, impact ImpactAssessment) correlation.Severity {
...
func (sibd *StorageIOBottleneckDetector) extractAffectedEntities(anomalies []*StorageAnomaly) []correlation.Entity {
...
func (sibd *StorageIOBottleneckDetector) assessDataQuality(events []correlation.Event, metrics map[string]correlation.MetricSeries) float64 {
	return 0.9
}