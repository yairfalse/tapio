...
type GroundTruthValidator struct {
...
func NewGroundTruthValidator(registry *PatternRegistry) *GroundTruthValidator {
...
func (pv *GroundTruthValidator) ValidatePattern(ctx context.Context, patternID string) (*ValidationRun, error) {
	pv.mutex.Lock()
	defer pv.mutex.Unlock()

	detector, err := pv.registry.Get(patternID)
	if err != nil {
...
func (pv *GroundTruthValidator) executeValidation(ctx context.Context, run *ValidationRun, detector types.PatternDetector) {
	defer func() {
...
func (pv *GroundTruthValidator) generateTestData(patternID string) (*TestData, error) {
	testData := &TestData{
		TestCases:   []*TestCase{},
...
func (pv *GroundTruthValidator) generateProductionTestCases(patternID string, count int) ([]*TestCase, error) {
...
func (pv *GroundTruthValidator) validateDetectionResult(testCase *TestCase, result *types.PatternResult) *ValidationResult {
	...
	if testCase.GroundTruth != nil && detected {
		...
		if result.RootCause != nil && testCase.GroundTruth.RootCause != "" {
			if rootCause, ok := result.RootCause.(*CausalityNode); ok {
				if rootCause.EventType == testCase.GroundTruth.RootCause {
					validationResult.CauseAccuracy = 1.0
				} else {
					validationResult.CauseAccuracy = 0.0
				}
			} else if rootCauseStr, ok := result.RootCause.(string); ok {
				if rootCauseStr == testCase.GroundTruth.RootCause {
					validationResult.CauseAccuracy = 1.0
				} else {
					validationResult.CauseAccuracy = 0.0
				}
...
func (pv *GroundTruthValidator) calculateAccuracyMetrics(run *ValidationRun) {
...
func (pv *GroundTruthValidator) updateValidationMetrics(run *ValidationRun) {
...
func (pv *GroundTruthValidator) calculateOverallMetrics() {
...
func (pv *GroundTruthValidator) GetValidationRun(runID string) (*ValidationRun, bool) {
	pv.mutex.RLock()
...
func (pv *GroundTruthValidator) ListValidationRuns() []*ValidationRun {
	pv.mutex.RLock()
	defer pv.mutex.RUnlock()
...
func (pv *GroundTruthValidator) GetValidationMetrics() *ValidationMetrics {
...
func (pv *GroundTruthValidator) ValidateAllPatterns(ctx context.Context) (map[string]*ValidationRun, error) {
