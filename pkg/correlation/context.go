...
type Context struct {
	...
	eventsBySource map[EventSource][]Event
	eventsByType   map[string][]Event
	eventsByEntity map[string][]Event
...
func NewContext(window TimeWindow, events []Event) *Context {
	ctx := &Context{
		Window:         window,
		events:         events,
...
func NewTestContext(events []Event) *Context {
	if len(events) == 0 {
		return NewContext(TimeWindow{
	...
	return NewContext(TimeWindow{Start: start, End: end}, events)
}
...
func (c *Context) entityKey(entity Entity) string {
	if entity.Namespace != "" {
...
func (c *Context) GetEventsBySource(source EventSource) []Event {
...
func (c *Context) GetEventsForEntity(entity Entity) []Event {
...
func (c *Context) GetMetric(name string) MetricSeries {
	return c.metrics[name]
}

// SetMetric adds a metric series to the context
func (c *Context) SetMetric(name string, series MetricSeries) {
...
func (c *Context) EntitiesRelated(e1, e2 Entity) bool {
	// Same entity
...
func (c *Context) CalculateFrequency(events []Event, unit time.Duration) float64 {
...
func (c *Context) DetectSpike(events []Event, baselineEvents []Event, threshold float64) bool {
...
func (c *Context) AnalyzeTrend(series MetricSeries) TrendAnalysis {
...
func (c *Context) GetUniqueNodes(events []Event) []string {
...
func (c *Context) GetUniquePods(events []Event) []string {
...
func (c *Context) GetUniqueNamespaces(events []Event) []string {
