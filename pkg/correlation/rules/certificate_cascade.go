...
func (r *CertificateCascadeRule) Execute(ctx context.Context, data *correlation.AnalysisData) ([]correlation.Finding, error) {
	...
	certIssues := r.checkCertificateIssues(data, windowStart)
	if len(certIssues) == 0 {
...
func (r *CertificateCascadeRule) checkCertificateIssues(data *correlation.AnalysisData, windowStart time.Time) []certIssue {
	...
	for _, secret := range data.KubernetesData.Secrets {
...
func (r *CertificateCascadeRule) checkWebhookFailures(data *correlation.AnalysisData, windowStart time.Time) []webhookFailure {
	var failures []webhookFailure

	// Check for webhook pods
	for _, pod := range data.KubernetesData.Pods {
		if !isAdmissionWebhook(pod) {
			continue
		}
					...
					for _, line := range logs {
						if strings.Contains(strings.ToLower(line), "certificate") ||
							...
							failures = append(failures, webhookFailure{
								PodName:      pod.Name,
								Namespace:    pod.Namespace,
								WebhookName:  extractWebhookName(pod),
...
func (r *CertificateCascadeRule) checkAdmissionErrors(data *correlation.AnalysisData, windowStart time.Time) []admissionError {
...
func (r *CertificateCascadeRule) checkDeploymentFailures(data *correlation.AnalysisData, windowStart time.Time) []deploymentFailure {
	...
	for _, rs := range data.KubernetesData.ReplicaSets {
...
func (r *CertificateCascadeRule) collectEvidence(certs []certIssue, webhooks []webhookFailure, admission []admissionError, deployments []deploymentFailure) []string {
	...
	for i, cert := range certs {
		if i >= 3 {
			evidence = append(evidence, fmt.Sprintf("... and %d more certificate issues", len(certs)-3))
			break
		}
...
// Utility functions
func extractWebhookName(pod types.PodInfo) string {
